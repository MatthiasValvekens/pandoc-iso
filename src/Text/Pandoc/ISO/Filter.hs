...
findTableIdInCaption =
  walkCaptionM findInInlines
    >=> walkCaptionM (return . stripWhiteSpace)
  where
    --findInInlines scans (and possibly modifies) a list of inlines
...
processNoteLikes' noteLikeType = do
  ...
  where
    -- no NOTE number if there's only one
...
processBlock blk@(Header lvl attrs headerText) = do
  ...
  newClause <-
    currentClause <<~ case curLvl - lvl of
      ...
      diff ->
        if diff < 0
          then -- descending more than one level at a time
          -- is not allowed
            throwError (LevelSkipped cls)
...
processBlock blk@(Div (divId', classes, kvals) blks)
  ...
  where
    registerNoteLike noteLikeType = do
      ...
      where
        idPref = identifierPrefix noteLikeType
        -- necessary to prevent the monomorphism restriction (?) from kicking in
        inClause :: Lens' RefBuildingState [Identifier]
        inClause = identifiersInClause noteLikeType
    ...
    prependEdNote = return $ Div (divId', classes, kvals) newBlks
      where
        newBlks = prependToBlocks [Str edNotePrefix] [Str ":", Space] blks
...
processInlineCitation refs cited = case refPrefix of
  ...
  where
    ...
    fmtInfo _ refType Nothing = Strong [Str errStr]
      where
        errStr = "!No " <> refType <> " labelled " <> refId <> "!"
...
substituteInlineRefs refs inl@(Cite [cited] _) = fromMaybe (return inl) maybeHandle
  where
    maybeHandle = processInlineCitation refs cited
...
substituteInlineRefs refs inl@(Cite cites _)
  ...
  where
    isNormRef cited = isJust (checkForPrefix "nrm" $ citationId cited)
...
handleStyles = walkPandocM (return . blockSub)
  where
    blockSub (Div attrs blks) = Div attrs' blks
      where
        attrs' = handleStyles' docxDivStyles attrs
    blockSub x = x
